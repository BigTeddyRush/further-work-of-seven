...
output_file="not_letter_counts_grouped.json"
stats_output_file="not_letter_counts_stats.json"
...
for json_file in ./results/*.json; do
    ...
    while IFS= read -r line; do
        # Check if the line does not contain "ProverResult.PROOF_FOUND"
        if ! echo "$line" | grep -q "ProverResult.PROOF_FOUND"; then
            ...
            # Only process and include non-zero letter counts
